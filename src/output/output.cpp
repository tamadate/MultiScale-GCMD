...
MD::exportDumpIn(void) {
	...
	for (auto &a : vars->Molecules[0].inAtoms) {
		fprintf(f,"%d %s %f %f %f %f %f %f %f\n",ID,vars->atypes[a.type].name.c_str(),a.qx-X,a.qy-Y,a.qz-Z,a.px,a.py,a.pz,1.0);
		ID++;
	}
	for (auto i : vars->MolID[1]) {
		for (auto &b : mols[i].inAtoms) {
			fprintf(f, "%d %s %f %f %f %f %f %f %f\n", ID, (vars->atypes[b.type].name+"(g)").c_str(), b.qx-X, b.qy-Y, b.qz-Z, b.px, b.py, b.pz, mols[i].w);
			ID++;
		}
	}
	for (auto i : vars->MolID[2]) {
		for (auto &b : mols[i].inAtoms) {
			fprintf(f, "%d %s %f %f %f %f %f %f %f\n", ID, (vars->atypes[b.type].name+"(v)").c_str(), b.qx-X, b.qy-Y, b.qz-Z, b.px, b.py, b.pz, mols[i].w);
			ID++;
		}
	}
	fclose(f);
